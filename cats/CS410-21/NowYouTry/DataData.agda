module NowYouTry.DataData where
...
natCode = `Σ bool λ b → if b then `⊤ else `X
  ...
  data Treeℕ : Set where
    leafOrNode : (b : Bool) -> (if b then ℕ  else Treeℕ × Treeℕ) -> Treeℕ

ArgForTreeℕ X = Σ[ b ∈ Bool ] (if b then nat else X × X)
treeℕCode = `Σ bool λ b → if b then ? else `X × `X
-}
...
module _ (U : Set)(El : U -> Set)(propU : {a : U} -> Irrelevant (El a))  where

  open DESC U El

  isPropArgs : {c : Desc}(d : Desc) -> Irrelevant (Args d ⟦ c ⟧Desc)
  ...
  genericIsProp : (d : Desc) -> Irrelevant (⟦ d ⟧Desc)
...
module _ where
  ...
  treeCode : Desc
  treeCode = `Σ bool λ b → if b then `Σ nat (λ _ → `⊤) else `X `× `X

  Treeℕ : Set
  Treeℕ = ⟦ treeCode ⟧Desc

  leaf : ℕ -> Treeℕ
  leaf n = con (true , n , tt)
