...
module _ {C : Category} where
  ...
  identityʳ (Kleisli M) {A} {B} {f} = C ⊧begin
    compSyn < f > (compSyn (fmapSyn (functor M) < return M B >) < join M B >)
      ≡⟦ solveCat refl ⟧
    < f > ；Syn -[ fmapSyn (functor M) < return M B > ；Syn < join M B > ]-
