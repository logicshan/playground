...
apply-twiceᵉ : (A : Type)
  ...
  → A
-- Exercise:
apply-twiceᵉ = λ (A : Type) (f : A → A) (x : A) → f (f x)

flipᵉ : (A : Type) → (B : Type) → (C : Type)
  → (A → B → C)
  → (B → A → C)
-- Exercise:
flipᵉ A B C f = λ b a → f a b
```
...
apply-twiceⁱ : {A : Type}
  ...
  → A
-- Exercise:
apply-twiceⁱ = λ f a → f (f a)

flipⁱ : {A : Type} → {B : Type} → {C : Type}
  → (A → B → C)
  → (B → A → C)
-- Exercise:
flipⁱ = λ f → λ b a → f a b
