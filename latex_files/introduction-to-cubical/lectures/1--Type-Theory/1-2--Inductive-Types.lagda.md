...
pos (suc x) ·ℤ m = m +ℤ (pos x ·ℤ m)
negsuc zero ·ℤ m = - m
negsuc (suc x) ·ℤ m = negsuc x ·ℤ m -ℤ m
...
double' : ℕ → ℕ
-- Exercise:
double' = ℕ-rec zero (λ x → suc (suc x))
```
...
-- Exercise:
ℕ-rec' a r zero = a
ℕ-rec' a r (suc n) = r n (ℕ-rec' a r n)
```
