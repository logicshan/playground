...
--  Exercise:
    to-fro f _ (inl a) = f (inl a)
    to-fro f _ (inr b) = f (inr b)
...
--  Exercise:
    to-fro (base , base) = refl
    to-fro (base , loop i) = refl
    to-fro (loop _ , base) = refl
    to-fro (loop _ , loop _) = refl

    fro-to : isRetract Torus→S¹×S¹ S¹×S¹→Torus
--  Exercise:
    fro-to torus-base = refl
    fro-to (torus-loop1 _) = refl
    fro-to (torus-loop2 _) = refl
    fro-to (torus-square _ _) = refl
```
...
×-map-≃ {A = A} {A' = A'} {B = B} {B' = B'} f g = packEquiv to sec to-fro ret fro-to
  where
    ...
    sec : A' × B' → A × B
--  Exercise:
    sec = ×-map (f .proof .section .map) (g .proof .section .map)

    ret : A' × B' → A × B
--  Exercise:
    ret = ×-map (f .proof .retract .map) (g .proof .retract .map)

    to-fro : isSection to sec
--  Exercise:
    to-fro (a' , b') = ×Path≃Path× .map
                         (f .proof .section .proof a' ,
                          g .proof .section .proof b')

    fro-to : isRetract to ret
--  Exercise:
    fro-to (a , b) = ×Path≃Path× .map
                       (f .proof .retract .proof a ,
                        g .proof .retract .proof b)
...
--  Exercise:
    ...
    to-fro : isSection to sec
--  Exercise:
    to-fro (inl a') = ap inl (f .proof .section .proof a')
    to-fro (inr b') = ap inr (g .proof .section .proof b')

    fro-to : isRetract to ret
--  Exercise:
    fro-to (inl a) = ap inl (f .proof .retract .proof a)
    fro-to (inr b) = ap inr (g .proof .retract .proof b)
```
...
```
    ret : (A → D) → (B → C)
--  Exercise:
    ret g = (e₂ .proof .retract .map) ∘ g ∘ (e₁ .proof .section .map)

    fro-to : isRetract to ret
--  Exercise:
    fro-to g i a = e₂ .proof .retract .proof (g (e₁ .proof .section .proof a i)) i
...
--  Exercise:
    ...
    fro-to : isRetract not not
--  Exercise:
    fro-to true = refl
    fro-to false = refl

sucℤ∘predℤ≡idfun : sucℤ ∘ predℤ ≡ idfun
sucℤ∘predℤ≡idfun i (pos zero) = pos zero
sucℤ∘predℤ≡idfun i (pos (suc x)) = pos (suc x)
sucℤ∘predℤ≡idfun i (negsuc x) = negsuc x

predℤ∘sucℤ≡idfun : predℤ ∘ sucℤ ≡ idfun
predℤ∘sucℤ≡idfun i (pos x) = pos x
predℤ∘sucℤ≡idfun i (negsuc zero) = negsuc zero
predℤ∘sucℤ≡idfun i (negsuc (suc x)) = negsuc (suc x)

sucℤ-≃ : ℤ ≃ ℤ
-- Exercise:
sucℤ-≃ = inv→equiv sucℤ predℤ (funext⁻ sucℤ∘predℤ≡idfun) (funext⁻ predℤ∘sucℤ≡idfun)
```
...
--  Exercise:
    ...
    to-fro (zeroˢ≡ i) = connection∧ zeroˢ≡ i
