...
take zero L = just []
take (suc n) [] = nothing
take (suc n) (x :: L) = λ pr → x :: take n L pr
...
∙-juxtapose-horizontal sq1 sq2 =
  flip-square (∙-juxtapose-vertical (flip-square sq1) (flip-square sq2))
```
...
-- Exercise:
diamond-tube p q i j k (i = i0) = p j
diamond-tube p q i j k (i = i1) = connection∧ q j k
diamond-tube p q i j k (j = i0) = p i
diamond-tube p q i j k (j = i1) = connection∧ q i k
diamond-tube p q i j k (k = i0) = connection∨ p i j

-- Exercise:
diamond p q i j = hcomp (∂ i ∨ ∂ j) (diamond-tube p q i j)
```
...
diamond-tube-alt : {x y z : A} (p : x ≡ y) (q : y ≡ z)
  → (i j k : I) → Partial (∂ i ∨ ∂ j ∨ ~ k) A
-- Exercise:
diamond-tube-alt p q i j k (i = i0) = p (j ∨ ~ k)
diamond-tube-alt p q i j k (i = i1) = q (j ∧ k)
diamond-tube-alt p q i j k (j = i0) = p (i ∨ ~ k)
diamond-tube-alt p q i j k (j = i1) = q (i ∧ k)
diamond-tube-alt {y = y} p q i j k (k = i0) = y
...
∙∙-filler-tube : {w x y z : A}
  ...
  → (i j k : I) → Partial (~ i ∨ ∂ j ∨ ~ k) A
-- Exercise:
∙∙-filler-tube r p q i j k (j = i0) = connection∧ (sym r) i k
∙∙-filler-tube r p q i j k (j = i1) = connection∧ q i k
∙∙-filler-tube r p q i j k (i = i0) = p j
∙∙-filler-tube r p q i j k (k = i0) = p j
...
-- Exercise:
∙-idl-tube {x = x} p i j k (i = i0) = p j  -- Constant in the `k` direction
∙-idl-tube {x = x} p i j k (j = i0) = x    -- Completely constant
∙-idl-tube {x = x} p i j k (j = i1) = p (~ i ∨ k) -- Constructed from `p` using a connection
∙-idl-tube {x = x} p i j k (k = i0) = p (~ i ∧ j) -- Constructed from `p` using connections in a different way
...
-- Exercise:
∙-invr-tube {x = x} p i j k (i = i1) = x
∙-invr-tube {x = x} p i j k (j = i0) = x
∙-invr-tube {x = x} p i j k (j = i1) = connection∨ (sym p) i k
∙-invr-tube {x = x} p i j k (k = i0) = p (~ i ∧ j)
...
∙-assoc-tube {w = w} r p q i j k (i = i0) = (r ∙ ∙-filler p q k) j
∙-assoc-tube {w = w} r p q i j k (i = i1) = ∙-filler (r ∙ p) q k j
∙-assoc-tube {w = w} r p q i j k (j = i0) = w
∙-assoc-tube {w = w} r p q i j k (j = i1) = q k
∙-assoc-tube {w = w} r p q i j k (k = i0) = (r ∙ p) j
...
```
invEquiv : A ≃ B → B ≃ A
invEquiv e@(equiv f (isEquivData (sectionData g s) (retractData g' r)))
