...
-- Exercise:
--zero≢suc p = subst (λ x → zero ≡ℕ x) p tt
zero≢suc p = subst (zero ≡ℕ_) p tt
```
...
inl≢inr {x = x} {y = y} path = subst IsInl path tt

inr≢inl : ¬ inr x ≡ inl y
-- Exercise:
inr≢inl path = subst IsInl (sym path) tt
```
...
--  Exercise: (Hint: use `J` again!)
    fro-to f = funext (λ y → funext (λ p → J P (J-refl Q (f x refl)) p))
      where
        P : (y : A) → x ≡ y → Type _
        P y p = J Q (f x refl) p ≡ f y p
```
...
≡≃≡ℕ x y = inv→equiv (encode x y) (decode x y) (to-fro x y) (fro-to x y)
  where
    ...
    fro-to-refl (suc x) = λ i j → suc (fro-to-refl x i j)
...
--  Exercise:
    decode (inl a) (inl b) = ap inl
    decode (inl a) (inr b) ()
    decode (inr a) (inl b) ()
    decode (inr a) (inr b) = ap inr

    to-fro : (x y : A ⊎ B) → isSection (encode x y) (decode x y)
--  Exercise:
    to-fro (inl x) (inl x₁) _ = refl
    to-fro (inl x) (inr x₁) ()
    to-fro (inr x) (inl x₁) ()
    to-fro (inr x) (inr x₁) _ = refl

    fro-to-refl : (x : A ⊎ B) → decode x x (encode x x refl) ≡ refl
--  Exercise:
    fro-to-refl (inl x) = refl
    fro-to-refl (inr x) = refl

    fro-to : (x y : A ⊎ B) → isRetract (encode x y) (decode x y)
--  Exercise:
    fro-to x y = J (λ z q → decode x z (encode x z q) ≡ q) (fro-to-refl x)
```
...
[]≢:: : ¬ ([] ≡ x :: xs)
[]≢:: {x = x} {xs = xs} p = subst IsNil p tt

::≢[] : ¬ (x :: xs ≡ [])
::≢[] {x = x} {xs = xs} p = subst IsNil (sym p) tt

-- injectivity of the cons constructor
::-inj₁ : ∀ {x y : A} {xs ys : List A} → x :: xs ≡ y :: ys → x ≡ y
::-inj₁ {x = x} p =
  subst (λ { (z :: _) → x ≡ z ; [] → ∅ }) p refl

::-inj₂ : ∀ {x y : A} {xs ys : List A} → x :: xs ≡ y :: ys → xs ≡ ys
::-inj₂ {xs = xs} p =
  subst (λ { (_ :: zs) → xs ≡ zs ; [] → ∅ }) p refl
...
≡≃≡List {A = A} xs ys = inv→equiv (encode xs ys) (decode xs ys) (to-fro xs ys) (fro-to xs ys)
  where
    code : (xs ys : List A) → Type
    code xs ys = xs ≡List ys
    ...
    encode xs ys = J (λ z _ → code xs z) (encode-refl xs)

    decode : (xs ys : List A) → code xs ys → xs ≡ ys
    decode [] [] c = refl
    decode [] (x :: ys) ()
    decode (x :: xs) [] ()
    decode (x :: xs) (y :: ys) (p , q) = ap-bin _::_ p (decode xs ys q)

    to-fro : (xs ys : List A) → isSection (encode xs ys) (decode xs ys)
    to-fro []       []        tt      = refl
    to-fro []       (_ :: _)  ()
    to-fro (_ :: _) []        ()
    to-fro (x :: xs) (y :: ys) (p , q) i = 
      ( J (λ y' p' → J (λ z _ → x ≡ z) refl p' ≡ p') 
          (J-refl (λ z _ → x ≡ z) refl) p i 
      , to-fro xs ys q i )

    fro-to-refl : (xs : List A) → decode xs xs (encode xs xs refl) ≡ refl
    fro-to-refl [] = refl
    fro-to-refl (x :: xs) i =
      λ j → (J-refl (λ z _ → x ≡ z) refl i j) :: (fro-to-refl xs i j)

    fro-to : (xs ys : List A) → isRetract (encode xs ys) (decode xs ys)
    fro-to xs ys p = J (λ z q → decode xs z (encode xs z q) ≡ q) (fro-to-refl xs) p
```
