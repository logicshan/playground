...
-- Exercise:
×-map-≃-underlying f g i (x , y) = (ua-comp f x i , ua-comp g y i)
```
...
-- Exercise:
+ℤᵘ≡+ℤ i m (pos zero) = m
+ℤᵘ≡+ℤ i m (pos (suc n)) = sucℤ (+ℤᵘ≡+ℤ i m (pos n))
+ℤᵘ≡+ℤ i m (negsuc zero) = predℤ m
+ℤᵘ≡+ℤ i m (negsuc (suc n)) = predℤ (+ℤᵘ≡+ℤ i m (negsuc n))
```
...
isEquiv-+ℤᵘ m = path→equiv (add-ℤ-≡ m) .proof
```
...
-- Exercise:
isEquiv-+ℤ m = subst isEquiv (λ i n → +ℤᵘ≡+ℤ i n m) (isEquiv-+ℤᵘ m)
```
...
refl≢loop p = true≢false (ap (λ q → subst double-cover q true) p)
```
...
iterateⁿ-predℤ-square p (pos (suc n)) i j = ∙-filler (iterateⁿ p (pos n)) p i j -- Try `∙-filler` for `p`
iterateⁿ-predℤ-square p (negsuc n)    i j = ∙-filler (iterateⁿ p (negsuc n)) (sym p) (~ i) j -- Try `∙-filler` for `sym p`
```
...
```
      ...
      decode-faces i y j k (j = i0) = base
      decode-faces i y j k (j = i1) = loop i
```
...
```
      -- Exercise:
      decode-faces i y j k (i = i0) = loopⁿ (sucℤ-≃ .proof .retract .proof y k) j
```
...
```
    ...
    -- Exercise:
    to-fro p = J (λ x q → decode x (encode x q) ≡ q) refl p
```
...
```
    ...
    -- Exercise:
    fro-to (pos zero)       = refl
    fro-to (pos (suc n))    = ap sucℤ (fro-to (pos n))
    fro-to (negsuc zero)    = refl
    fro-to (negsuc (suc n)) = ap predℤ (fro-to (negsuc n))
```
...
_+ℤᵐ_ : ℤ → ℤ → ℤ
-- Exercise:
x +ℤᵐ y = ΩS¹≃ℤ .map (λ i → loopⁿ x i ·S¹ loopⁿ y i)
