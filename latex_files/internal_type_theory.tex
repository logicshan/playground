...
\begin{frame}{背景}
  每个拓扑斯都拥有一种强大的内部语言, 它可以有多种呈现方式. 最常见的呈现方式是基于多类高阶逻辑, 通常被称为米切尔-贝纳布语言. 不过, 我们采用一种基于外延马丁-洛夫类型理论的呈现方式.
...
\begin{frame}[fragile,shrink]{扩展上下文$\Gamma.(A,a)$}
  \[
  \begin{tikzcd}[row sep=large]
    \Gamma.(A,a) \arrow[dr,phantom,"\lrcorner",very near start] \arrow[d] \arrow[r,tail,"\bar{a}"] & \Gamma\times A \arrow[d,"a"] \\
    1 \arrow[r,"\top"] & \Omega
  \end{tikzcd}
  \]
  扩展上下文 $Γ.(A, a)$ 简单地由 $a$ 分类的子对象给出, 即 $\bar{a} : Γ.(A, a) \rightarrowtail Γ × A$ 的域.
  \[
  \begin{tikzcd}
    & \Gamma.(A,a) \arrow[d,tail,"\bar{a}"] \arrow[dl,"\ttp"'] \arrow[dr,"\ttq"] & \\
    \Gamma & \Gamma\times A \arrow[l,"\fst"] \arrow[r,"\snd"'] & A
  \end{tikzcd}
  \]
  $\ttp \triangleq \fst\circ\bar{a} \qquad \ttq \triangleq \snd\circ\bar{a}$
\end{frame}
...
& \Gamma.(A,a) \arrow[dr,phantom,"{\lrcorner}",very near start] \arrow[r,tail,"{\bar{a}}"] \arrow[d]
...
\end{tikzcd}
  ...
  $\ttp\circ\langle\gamma,t\rangle = \fst\circ\bar{a}\circ\langle\gamma,t\rangle = \fst\circ(\gamma,t) = \gamma$

  $\ttq[\langle\gamma,t\rangle] = \ttq\circ\langle\gamma,t\rangle = \snd\circ\bar{a}\circ\langle\gamma,t\rangle=\snd\circ(\gamma,t)=t$
\end{frame}
...
\begin{frame}[fragile,shrink]{内涵子类型在 CwF 中的解释}
  ...
  \begin{tabular}{cc}
  \begin{tikzcd}
    \cdot \arrow[r,tail,"\bar{\varphi}"] \arrow[d] & \Gamma.(A,a) \arrow[d,"{\varphi}"] \\
    1 \arrow[r] & \Omega
  \end{tikzcd}
  &
  \begin{tikzcd}
    \Gamma.(A,a) \arrow[r,tail,"\bar{a}"] \arrow[d] & \Gamma\times A \arrow[d,"a"] \\
    1 \arrow[r] & \Omega
  \end{tikzcd}
\end{tabular}
\]
更准确地说, 我们有$\varphi:\Gamma.(A,a)\to\Omega$分类单态射$\bar{\varphi}:\cdot\rightarrowtail\Gamma.(A,a)$, $a$ 分类单态射 $\bar{a}:\Gamma.(A,a)\rightarrowtail\Gamma\times A$, 因此$\bar{a}\circ\bar{\varphi}:\cdot\rightarrowtail\Gamma\times A$. 我们取$a'=\chi_{\bar{a}\circ\bar{\varphi}}:\Gamma\times A\to\Omega$.
\end{frame}

\begin{frame}[fragile,shrink]{内涵子类型引入和消除规则的合理性}
  \begin{center}
  \begin{tikzcd}
    \Gamma.(A,a')\arrow[drr,phantom,"\lrcorner"{very near start,description}]\arrow[r,tail,"\bar{\varphi}"]\arrow[d] & \Gamma.(A,a)\arrow[r,tail,"\bar{a}"] & \Gamma\times A \arrow[d,"a'"] \\
    1 \arrow[rr,"\top"] & & \Omega
  \end{tikzcd}
  \quad$\stackrel{\bar{a}\text{是单态射}}{\implies}$\quad
  \begin{tikzcd}
    \Gamma.(A,a')\arrow[dr,phantom,"\lrcorner" very near start]\arrow[r,tail,"\bar{\varphi}"] \arrow[d] & \Gamma.(A,a) \arrow[d,"a'\circ\bar{a}"] \\
    1 \arrow[r,"\top"] & \Omega
  \end{tikzcd}
  \end{center}
...
\begin{frame}[fragile,shrink]{$t\in\Ter(\Gamma\vdash(A,a'))$}
  ...
  \]
  \[
  a'\circ(id,t)=a'\circ\bar{a}\circ\langle id,t\rangle=\varphi\circ\langle id,t\rangle=\top\circ1_{\Gamma}
  \]
\end{frame}
\end{document}