...
\begin{frame}{$\Sigma$-项(term)}
 不同变量$x_1, \ldots, x_k$的一个列表(可能为空)被称为上下文(context)

 \vspace{6mm}

 在上下文$x_1, \ldots, x_k$中的$\Sigma$-项根据以下规则归纳定义
 \begin{enumerate}
  \item 在上下文$x_1, \ldots, x_k$中, 每一个变量 $x_i$ 都是一个 $\Sigma$-项
  \item 如果$t_1, \ldots, t_{\arity{i}}$是上下文$x_1, \ldots, x_k$中的$\Sigma$-项， 那么 $\op{i}(t_1, \ldots, t_{\arity{i}})$ 是上下文 $x_1, \ldots, x_k$中的一个$\Sigma$-项
 \end{enumerate}
 ...
 \begin{equation*}
  x_1, \ldots, x_k \mid t
\end{equation*}
表示$t$是上下文$x_1, \ldots, x_k$中的一个$\Sigma$-项
...
\begin{frame}{$\Sigma$-等式}
  一个$\Sigma$-等式就是上下文$x_1, \ldots, x_k$中的一对$\Sigma$-项$\ell$和$r$. 一般记为
\begin{equation*}
...
\begin{enumerate}
  ...
  \end{equation*}
  (为了简单起见, 我们在合适的地方省略了上下文$x_1, \ldots, x_k$)
\end{enumerate}
\end{frame}
