...
为了从这些路径类型得到具有标准定义性（而非命题性）计算性质的 Martin-Löf 恒等类型，我们使用 Swan 构造 [60] 的一个版本，类似于 [18] 第 9.1 节中的版本，但只使用 $\mathtt{I}$ 上的连接代数结构，而非 De Morgan 代数结构。这是唯一使用公理 $\ax{7}$ 的地方；我们需要由 $\mathtt{Cof}$ 和 $[\_] : \mathtt{Cof} \to \mathcal{U}$ 给出的宇宙在依赖积下封闭这一事实：
...
\begin{equation}
\mathtt{Glue}\ \Phi\,A\,B\,f\ x \triangleq \mathtt{Glue}\ (\Phi\ x)\ (A(x, \_)))\ (B\ x)\ (f(x, \_))
\end{equation}
\end{definition}
...
\[
\varepsilon\ x\ (\varphi, f) \triangleq \alpha\ 0\ (\lambda\_ \to x)\ \varphi\ ((p\ x) \circ f)\ (a_0\ x)
\]
...
\[
b_1 \triangleq \beta\ 1\ (\lambda(\_ : \ttI) \to x)\ (\Phi\ x \lor \psi)\ (p_b \cup (\mathtt{refl} \circ \tilde{b_1}))\ b_1'
\]
...
\[
\mathtt{glue}\ (x, u)\ a \triangleq (\lambda\_ \to a, f\ x\ u\ a)
\]
...
定义 CCHM 纤维化之间的路径类型 $\_\sim_{\mathcal{U}}\_ : \{\Gamma : \mathcal{U}\} \to \mathtt{Fib}\ \Gamma \to \mathtt{Fib}\ \Gamma \to \mathcal{U}_1$ 为
\[
...
容易验证这两个构造是互逆的。因此由 $\_\sim_{\mathcal{U}}\_$ 描述的数据恰好对应于描述宇宙中路径所需的数据。
...
&r\ x\ a\ i \triangleq \rho\ 1\ \langle x,  id \rangle\ (i = 0 \lor i = 1)\ ((\lambda\_ \to p\ x\ a) \cup (\lambda\_ \to q\ x\ (f\ x\ a)))\ (f\ x\ a)\\
&s : (x : \Gamma)(b : B\ x) \to b \sim f\ x\ (g\ x\ b)\\
&s\ x\ b\ i \triangleq \rho\ 0\ \langle x,  id \rangle\ (i = 0 \lor i = 1)\ ((\lambda\_ \to q\ x\ b) \cup (\lambda\_ \to p\ x\ (g\ x\ b)))\ (g\ x\ b)
\end{align*}
...
&f'\ (x, i) \triangleq (\lambda\_ : [i = 0] \to f\ x) \cup (\lambda\_ : [i = 1] \to  id )
\end{align*}

现在令 $P \triangleq \mathtt{SGlue}\ \Phi\ C\ (\lambda(x, \_) \to B\ x)\ f'$ 并观察到由 $\mathtt{SGlue}$ 的严格性性质，$P(x, 0) = A\ x$ 和 $P(x, 1) = B\ x$。
...
\[
(P, \rho)[\iota : (\Gamma \times \ttI)|\Phi \hookrightarrow \Gamma \times \ttI] = (C, \gamma)
\]
...
回忆 $\rho$ 是 $\mathtt{SGlue}$ 的合成结构，我们可以计算 $\rho\ 0\ \langle x,  id \rangle\ \bot\ \mathtt{elim}_{\emptyset}\ a$。我们首先跨越与 $\mathtt{Glue}$ 的同构，使得 $a$ 变为 $(\lambda\_ \to a, f\ x\ a)$。我们现在追踪 $\mathtt{Glue}$ 中的合成算法：我们开始在 $B\ x$ 中合成以得到 $b_1' \triangleq \beta\ 0\ (\lambda\_ \to x)\ \bot\ \mathtt{elim}_{\emptyset}\ (f\ x\ a)$。接下来，我们使用等价结构来推导出 $a_1 : (u : [1 = 0 \lor 1 = 1]) \to B\ x$ 和 $p_b : (u : [1 = 0 \lor 1 = 1]) \to  id (a_1\ u) \sim b_1'$，其中特别地，$a_1$ 由 $a_1\ u \triangleq \beta\ 1\ (\lambda\_ \to x)\ \bot\ \mathtt{elim}_{\emptyset}\ b_1'$ 给出。然后我们执行合成的最后一步，即从 $b_1'$ 在 $B\ x$ 中合成以得到 $b_1$。这给我们留下了在 $\mathtt{Glue}$ 中合成的结果 $(a_1, b_1)$。当跨越同构传回时，我们简单地取对的第一分量，即 $a_1$，但现在被视为一个全元素，以得到在 $\mathtt{SGlue}$ 中合成的最终结果 $\beta\ 1\ (\lambda\_ \to x)\ \bot\ \mathtt{elim}_{\emptyset}\ b_1'$。因此，总结起来我们有：
\begin{align*}
\mathtt{coerce}\ (P, \rho)\ x\ a &= \rho\ 0\ \langle x,  id \rangle\ \bot\ \mathtt{elim}_{\emptyset}\ a\\
&= \beta\ 1\ (\lambda\_ \to x)\ \bot\ \mathtt{elim}_{\emptyset}\ b_1'\\
&= \beta\ 1\ (\lambda\_ \to x)\ \bot\ \mathtt{elim}_{\emptyset}\ (\beta\ 0\ (\lambda\_ \to x)\ \bot\ \mathtt{elim}_{\emptyset}\ (f\ x\ a))
\end{align*}
...
回到具有其相关预层拓扑斯 $\widehat{\mathbf{C}}$ 的小范畴 $\mathbf{C}$ 的一般情况，如果我们取区间对象 $\ttI \in \widehat{\mathbf{C}}$ 为某个对象 $\mathbf{i} \in \mathbf{C}$ 的可表函子 $\mathtt{y}_i \triangleq \mathbf{C}(\_, i)$，那么以下定理给出了这种区间对象满足公理 $\ax{1}$ 的一个有用判据。

\begin{theorem}
在预层拓扑斯 $\widehat{\mathbf{C}}$ 中，可表函子 $\ttI = \mathtt{y}_i$ 满足公理 $\ax{1}$，如果 $\mathbf{C}$ 是一个\textbf{余筛选范畴}，即如果 $\mathbf{Set}$ 中的有限积与 $\mathbf{C}^{\mathrm{op}}$ 上的余极限交换 [26]。
\end{theorem}
...
\begin{align*}
\widehat{\mathbf{C}}(\mathtt{y}_c \times \mathtt{y}_i, \Delta\{0,1\}) &\cong \mathbf{Set}(\operatorname{colim}_{\mathbf{C}^{\mathrm{op}}}(\mathtt{y}_c \times \mathtt{y}_i), \{0,1\}) \\
&\cong \mathbf{Set}(\operatorname{colim}_{\mathbf{C}^{\mathrm{op}}} \mathtt{y}_c \times \mathtt{y}_i, \{0,1\}) \\
&\cong \mathbf{Set}(1 \times 1, \{0,1\}) \cong \{0,1\}
\end{align*}
由于 $\widehat{\mathbf{C}}$ 中的可判定子对象由 $1 + 1 = \Delta\{0,1\}$ 分类，这意味着 $\mathtt{y}_c \times \mathtt{y}_i$ 只有两个可判定子对象：最小和最大子对象。由于这对所有 $c \in \mathbf{C}$ 成立，因此 $\ttI = \mathtt{y}_i$ 满足 $\ax{1}$。
\end{proof}

余筛选性的一个更初等的刻画是：$\mathbf{C}$ 是居住的，并且对于每对对象 $c, c' \in \mathbf{C}$，跨度范畴 $c \leftarrow \cdot \rightarrow c'$ 是连通范畴 [2, 定理 2.15]。任何具有有限积的范畴显然具有这一性质。这对于上面定义的立方体范畴 $\square$ 成立，因此 [18] 中的模型（其中 $\mathbf{C} = \square$ 且 $i$ 是泛 De Morgan 代数）的区间满足 $\ax{1}$。不具有有限积但仍然是余筛选的一个相关例子是 $\Delta$，居住有限线性有序集 $[0 < 1 < \cdots < n]$ 的范畴，其 $\widehat{\Delta}$ 是\textbf{单纯形集}范畴，在同伦论中被广泛使用 [28]。因此 $\widehat{\Delta}$ 中区间的自然候选，即当 $i$ 是 1-单纯形 $[0 < 1]$ 时的 $\mathtt{y}_i$，满足 $\ax{1}$。
...
在具有区间对象的拓扑斯中，有许多候选的子对象 $\mathtt{Cof} \hookrightarrow \Omega$ 满足图 5.4 中的公理 $\ax{5}$--$\ax{8}$。在一个极端，我们可以取 $\mathtt{Cof}$ 为整个 $\Omega$。在相反的极端，我们可以取（内部地）由以下要求归纳定义的子对象：它包含命题 $i = 0$ 和 $i = 1$（对于所有 $i : \ttI$）并且对二元析取、依赖和 $\ttI$-索引合取封闭，从而获得满足 $\ax{5}$--$\ax{8}$ 的最小 $\mathtt{Cof}$。然而，余纤维命题还必须满足严格性公理 $\ax{9}$，我们接下来考虑这一点。
...
如果 $\mathcal{S}$ 是环境集合论中的一个 Grothendieck 宇宙，那么它在该 CwF 中的 Hofmann-Streicher 提升 [31] 到一个宇宙 $\mathcal{U}$ 满足：$\widehat{\mathbf{C}}$ 中的态射 $\Gamma \to \mathcal{U}$ 命名取值于 $\mathcal{S}$ 的族 $(\int \Gamma)^{\mathrm{op}} \to \mathcal{S}$。

\begin{definition}[$\Omega_{\mathrm{dec}}$]
预层拓扑斯 $\widehat{\mathbf{C}}$ 中的子对象分类器 $\Omega$ 将每个 $c \in \mathbf{C}$ 映射到 $c$ 上的筛集合 $\Omega(c)$，即前复合封闭的子集 $S \subseteq \mathrm{obj}(\mathbf{C}/c)$。令 $\Omega_{\mathrm{dec}} \hookrightarrow \Omega$ 为子预层，其在每个 $c \in \mathrm{obj}\ \mathbf{C}$ 处的值是 $\Omega(c)$ 中由 $\mathrm{obj}(\mathbf{C}/c)$ 的可判定子集构成的那些筛组成的子集。
\end{definition}

当然，如果环境集合论满足排中律，那么 $\Omega_{\mathrm{dec}} = \Omega$。一般来说，$\Omega_{\mathrm{dec}}$ 分类 $\widehat{\mathbf{C}}$ 中的单态射 $\alpha : F \hookrightarrow G$，使得对于所有 $c \in \mathrm{obj}\ \mathbf{C}$，（单射）函数 $\alpha_c : F\ c \to G\ c$ 具有可判定像。

\begin{theorem}
将宇宙 $\mathcal{U}$ 解释为 $\mathbf{Set}$ 中 Grothendieck 宇宙的 Hofmann-Streicher 提升 [31]，预层拓扑斯 $\widehat{\mathbf{C}}$ 中的子对象 $\mathtt{cof} \hookrightarrow \Omega$ 满足严格性公理 $\ax{9}$，如果它包含在 $\Omega_{\mathrm{dec}} \hookrightarrow \Omega$ 中。
\end{theorem}
...
\begin{corollary}
令 $\mathbf{C}$ 为具有有限积的小范畴，包含一个具有非平凡连接代数结构 $0, 1 : 1 \to i, \sqcap, \sqcup : i \times i \to i$（参见图 5.4）的对象 $i$。假设对于每个对象 $c \in \mathbf{C}$，集合 $\mathbf{C}(c, i)$ 具有可判定等式。那么如果我们取区间 $\ttI$ 为 $\mathtt{y}_i$，$\mathtt{Cof}$ 为 $\Omega_{\mathrm{dec}}$，预层拓扑斯 $\widehat{\mathbf{C}}$ 满足图 5.4 中的所有公理。
\end{corollary}

\begin{proof}
我们已经在第 5.6.1 节中注意到，这种 $\ttI$ 的选择满足公理 $\ax{1}$--$\ax{4}$。每个集合 $\mathbf{C}(c, i)$ 的可判定性意味着 $\widehat{\mathbf{C}}$ 中的子对象 $\{0\} \hookrightarrow \ttI$ 和 $\{1\} \hookrightarrow \ttI$ 通过 $\Omega_{\mathrm{dec}} \hookrightarrow \Omega$ 分解，因此当 $\mathtt{Cof} = \Omega_{\mathrm{dec}}$ 时公理 $\ax{5}$ 得到满足。注意这种 $\mathtt{Cof}$ 的选择自动满足 $\ax{6}$ 和 $\ax{7}$；并且由定理 5.6.3 它满足公理 $\ax{9}$。因此只需检查公理 $\ax{8}$ 是否满足。我们在引理 5.1.5(ii) 中看到，这个公理等价于要求余纤维在由 $\ttI$ 取指数下封闭。在这种情况下，余纤维是由 $\Omega_{\mathrm{dec}}$ 分类的单态射 $\alpha : F \hookrightarrow G$，我们在定义 5.6.2 之后注意到它们由每个函数 $\alpha_c \in \mathbf{Set}(F\ c, G\ c)$ 具有可判定像这一事实刻画。这些单态射在由 $\ttI$ 取指数下的封闭性来自于 $\mathbf{C}$ 具有有限积且 $\ttI = \mathtt{y}_i$ 是可表的事实；因为那时 $\ttI \to (\_)$ 同构于由 $(\_) \times i : \mathbf{C} \to \mathbf{C}$ 预复合诱导的函子 $\widehat{\mathbf{C}} \to \widehat{\mathbf{C}}$，这显然保持逐分量可判定像性质。
\end{proof}
...
\begin{proof}
我们已经在第 5.6.1 节中注意到，这种 $\ttI$ 的选择满足公理 $\ax{1}$--$\ax{4}$。如果 $\mathtt{Cof} = \Omega$（即 $\mathtt{cof} = \lambda\_ \to \top$），那么公理 $\ax{5}$--$\ax{8}$ 平凡成立。此外，如果 LEM 成立，那么 $\Omega_{\mathrm{dec}} = \Omega$，因此由定理 5.6.3 公理 $\ax{9}$ 成立。
\end{proof}

\begin{remark}
作为定理 5.6.3 的部分逆，我们有如果 $\ax{9}$ 被与 $\widehat{\mathbf{C}}$ 相关联的 CwF 中的 Hofmann-Streicher 宇宙满足，那么每个余纤维单态射 $\alpha : F \hookrightarrow G$ 的分量函数 $\alpha_c \in \mathbf{Set}(F\ c, G\ c)$（$c \in \mathrm{obj}\ \mathbf{C}$）的像是 $\neg\neg$-闭子集。为看到这一点，我们可以应用 Andrew Swan 的一个论证 [私人通信]，它依赖于环境集合论中有
\begin{equation}
...
由于在构造性设定中自由 De Morgan 代数的等式是可判定的，因此当 $\mathbf{C} = \square$（立方体范畴，定义 3.3.1）时推论 5.6.4 给出了我们公理的一个模型。然而，这使用了与 [18, 第 4.1 节] 中不同的余纤维性选择。在本节剩余部分，我们检查 CCHM 的余纤维性概念是否满足我们的公理。
...
由于自由 De Morgan 代数 $\mathsf{dM}(I)$ 作为分配格由符号 $i$ 和 $1-i$（当 $i$ 遍历有限集 $I$）自由生成，我们可以将 $\mathbb{F}(I)$ 视为 $\mathsf{dM}(I)$ 的商格，通过将 $i$ 映射到 $(i = 1)$、$1-i$ 映射到 $(i = 0)$ 的函数；我们记 $q_I : \mathsf{dM}(I) \to \mathbb{F}(I)$ 为商映射。不难看到，对于每个 $f \in \mathbf{C}(J, I)$，相应的 De Morgan 代数同态 $\mathsf{dM}(I) \to \mathsf{dM}(J)$（我们也记为 $f$）诱导面格之间的格同态：
\[
\begin{tikzcd}
\mathsf{dM}(I) \arrow[r] \arrow[d, "q_I"'] & \mathsf{dM}(J) \arrow[d, "q_J"] \\
\mathbb{F}(I) \arrow[r, "\mathbb{F}f"'] & \mathbb{F}(J)
\end{tikzcd}
\]
这使得 $\mathbb{F}$ 成为 $\square$ 中的一个对象，并且存在单态射 $m : \mathbb{F} \hookrightarrow \Omega$，其在 $I \in \mathrm{obj}\ \square$ 处的分量将每个 $\varphi \in \mathbb{F}(I)$ 发送到筛
\[
m_I(\varphi) = \{\cdot \xrightarrow{f} I \mid \mathbb{F}f\ \varphi = \top\}
\]

我们现在在 $\hatcube$ 中取由这个单态射 $m : \mathbb{F} \hookrightarrow \Omega$ 给出的子对象 $\mathtt{Cof} \hookrightarrow \Omega$。公理 $\ax{1}$--$\ax{4}$ 无需改变即可由推论 5.6.4 得出；公理 $\ax{5}$--$\ax{7}$ 由面格 $\mathbb{F}(I)$ 的定义得出。因此只需检查公理 $\ax{8}$ 和 $\ax{9}$。

回忆 $\hatcube$ 中的区间对象是单元素子集 $\{i\} \in \mathrm{obj}\ \square$ 上的可表预层 $\ttI = \mathtt{y}_{\{i\}}$。对于任意对象 $I \in \mathrm{obj}\ \square$，设其中有 $n$ 个不同元素 $i_1, \ldots, i_n \in \mathbb{D}$，可表 $\mathtt{y}_I$ 同构于 $\hatcube$ 中的 $n$ 重积 $\ttI^n$。因此筛 (5.53) 对应于 $n$-立方体 $\ttI^n$ 的子对象。确实，面格 $\mathbb{F}(I)$ 中的每个元素是有限个不可约元素的有限并；每个不可约元素是形如 $i = 0$ 或 $j = 1$ 的原子条件的有限合取。$\ttI^n$ 的相应子对象是面的有限并，即通过将某些坐标设为 0 或 1 得到的 $\ttI^n$ 的子对象。$\mathbb{F}(I)$ 元素的这种析取范式意味着其等式是可判定的，因此这个 $\mathtt{Cof}$ 包含在 $\Omega_{\mathrm{dec}}$ 中且公理 $\ax{9}$ 得到满足（定理 5.6.3）。最后，公理 $\ax{8}$ 来自于余纤维沿投影 $\pi_1 : \ttI^n \times \ttI \to \ttI^n$ 的拉回具有（稳定的）右伴随，或者等价地，格态射 $\mathbb{F}(I) \to \mathbb{F}(I \cup \{i\})$（对于任意 $I \in \mathrm{obj}\ \square$ 和 $i \in \mathbb{D} - I$）具有（稳定的）右伴随这一事实。这是面格的\textbf{量词消去}结果；参见 [18, 引理 2]。
\end{definition}

\section{相关工作}

本章呈现的工作受到 Coquand [19] 的建议启发，即 [18] 中发展的某些构造可能使用拓扑斯的内部逻辑来更好地理解。我们展示了如何在拓扑斯的内部类型论中表达 Cohen、Coquand、Huber 和 Mörtberg 的纤维化概念。与例如 Gambino 和 Sattler [27, 第 3 节] 为同样目的使用的更抽象的弱分解系统和模型范畴的范畴论方法相比，内部语言的使用允许一个令人愉悦的简单描述（定义 5.3.3）。Birkedal 等人 [12] 发展了守卫立方类型论，其语义基于预层拓扑斯内部逻辑中 [18] 的公理化版本。

在配备有类似区间对象的拓扑斯的框架内，我们发现相当简单的公理集合（图 5.4）足以使其建模具有满足弱形式泛等性的内涵恒等类型的 Martin-Löf 类型论。特别地，区间上只需要简单的连接代数结构，而非 De Morgan 代数结构。此外，适合一致 Kan 填充的命题集合不是紧密约束的，可以以多种方式选择。在第 5.6 节中，我们只考虑了预层范畴如何满足我们的公理。考虑一般 Grothendieck 拓扑斯（其中预层被限制为给定覆盖概念的层）中的模型可能是有趣的，特别是如 Johnstone 的拓扑拓扑斯 [36] 这样的大拓扑斯；这允许区间对象是（对应于）通常的拓扑区间，因此类型论的模型与空间的经典同伦类型有相当直接的联系。然而，尽管 Hofmann-Streicher [31] 的宇宙构造（[18] 第 8.2 节中满足完全泛等性公理的纤维化宇宙构造的基础）可以通过使用层化 [59, 第 3 节] 从预层拓扑斯扩展到层拓扑斯，但层化似乎与 CCHM 的纤维化概念交互不佳。在另一个方向上，Frumin 和 Van Den Berg [25] 的最新工作使用我们的初等、公理化方法，使用非 Grothendieck 拓扑斯，即有效拓扑斯 [35]。最后，Uemura 也应用这种方法证明了立方类型论中命题调整公理的独立性。
\end{document}