...
在本节中，我们给出 Cohen 等人 [18] 提出的\textbf{粘合}构造的内部表述。粘合类似于类型族的合成结构（定义 5.3.3），只不过它涉及的不是类型的部分路径，而是类型之间的部分等价。粘合对于 5.5 节中给出的与泛等性 [62] 相关的构造至关重要。
...
对于 $A : \Gamma \to \mathcal{U}$ 的扩展结构类似于 $A$ 的合成结构，在这个意义上两者都允许我们扩展部分元素；事实上，每个具有扩展结构的族都是一个纤维化。然而，扩展结构不需要一个全元素作为我们扩展/合成的起点，因此实际上是一个比合成结构更强的概念。首先注意，对于 $A : \mathcal{U}$ 具有扩展结构意味着 $A$ 是居住的，因为我们总是可以扩展空的部分元素。此外，给定任何元素 $a : A$，我们可以使用扩展结构来证明它与空部分元素的扩展是路径相等的。这些事实一起告诉我们 $A$ 是\textbf{可收缩的}：

\begin{definition}[可收缩性，参见定义 2.1.1]
如果类型 $A$ 有一个收缩中心 $a_0 : A$ 并且每个元素 $a : A$ 都命题相等于 $a_0$，即存在一条路径 $a_0 \sim a$，则称类型 $A$ 是\textbf{可收缩的}。因此，如果 $\mathtt{Contr}\ A$ 是居住的，则类型是可收缩的，其中 $\mathtt{Contr} : \mathcal{U} \to \mathcal{U}$ 定义为
\[
...
\begin{definition}[等价 {\cite[第 4.4 节]{62}}]
给定类型 $A, B : \mathcal{U}$，函数 $f : A \to B$ 是一个\textbf{等价}，如果类型 $\mathtt{Equiv}\ f$ 是居住的，其中
\[
...
&\quad(f : (x : \Gamma)(u : [\Phi\ x]) \to A(x, u) \to B\ x) \to\notag\\
&\quad((x : \Gamma)(v : [\Phi\ x]) \to \mathtt{Equiv}(f\ x\ v)) \to\notag\\
&\quad\mathtt{isFib}\ A \to \mathtt{isFib}\ B \to \mathtt{isFib}(\mathtt{Glue}\ \Phi\ A\ B\ f)\notag
\end{align}
\end{theorem}
...
我们首先通过在 $B$ 中沿 $p$ 合成来得到 $b_1' : B\ x$，这可以看作是 $b_1$ 的一个初步近似：
\[
b_1' \triangleq \beta\ e\ p\ \psi\ (\lambda(v : [\psi])(i : \ttI) \to \mathtt{snd}(q\ v\ i))\ b_0
\]
回忆 $a_1$ 将具有类型 $(u : [\Phi\ x]) \to A(x, u)$，因此我们假设 $u : [\Phi\ x]$ 以定义一个 $A(x, u)$ 类型的元素。注意我们不能简单地在 $A$ 中沿 $p$ 合成，因为我们不知道 $\Phi(p\ i)$ 对所有 $i : \ttI$ 成立。相反，我们将使用等价结构来定义 $a_1$。
...
\]
这是良定义的，因为如上所述，我们有 $\forall(v : [\psi]).\ f\ x\ u\ (\tilde{a_1}\ v\ u) = \tilde{b_1}\ v$，并且由合成结构的类型，我们有 $(\psi, \tilde{b_1}) \nearrow b_1'$，因此给定 $v : [\psi]$ 我们有 $\mathtt{refl}(\tilde{b_1}\ v) : f\ x\ u\ (\tilde{a_1}\ v\ u) \sim b_1'$。现在我们可以定义
\[
...
我们现在有 $a_1$ 和 $b_1'$ 使得 $\tilde{a_1} \nearrow a_1$ 和 $\tilde{b_1} \nearrow b_1'$。然而，我们不能简单地取 $b_1$ 为 $b_1'$，因为我们不知道 $\forall(u : \Phi\ x).\ f\ x\ u\ (a_1\ u) = b_1'$，因此不能得出 $(a_1, b_1') : (\mathtt{Glue}\ \Phi\ A\ B\ f)\ x$。为了解决这个问题，我们在 $B\ x$ 中执行最后一次合成以``修正''$b_1'$ 以实现这一性质。考虑以下连接
\[
p_b \cup (\mathtt{refl} \circ \tilde{b_1}) : [\Phi\ x \lor \psi] \to \Pi_{\ttI}(B\ x)
\]
这是良定义的，因为 $p_b$ 是通过扩展 $\mathtt{refl} \circ \tilde{b_1}$ 定义的，因此它们在共同定义的地方必须相等。我们用这个在 $B\ x$ 中执行最后一次合成：
\[
...
我们现在有一种方法来解释 \cite{18} 中的粘合操作，它满足一些必要的要求；参见 \cite[图 4]{18}。然而，当前的构造未能满足沿包含 $\iota : \Gamma|\Phi \hookrightarrow \Gamma$ 重索引时 $\mathtt{Glue}\ \Phi\ A\ B\ f$ 应该等于 $A$ 的要求。事实上，这个等式应该在 CCHM 纤维化的 CwF 中成立。这意味着不仅 $A = (\mathtt{Glue}\ \Phi\ A\ B\ f) \circ \iota : \Gamma|\Phi \to \mathcal{U}$，而且重索引定理 5.4.7 中导出的纤维化结构应该得到与我们开始时相同的纤维化结构。精确地说，我们所要求的是：
\begin{equation}
...
\begin{remark}
除了使用拓扑斯的内部语言之外，我们获得具有良好性质的粘合操作的方法与 Cohen 等人 \cite{18} 的方法不同，在那里粘合是直接以所有所需性质定义的。然而，可以看到我们最后两个步骤在原始工作中的位置。严格化可以在 \cite[定义 15]{18} 中对 $\varphi\rho = 1_\mathbb{F}$ 的情况分裂中看到；更多细节参见第 5.6.2 节。Cohen 等人不是为粘合定义一个初始合成结构然后修改它以获得所需的重索引性质，而是直接定义合成结构。从 \cite[第 6.2 节]{18} 中移除所有 $\forall$ 算子的使用将得到我们的初始合成结构，然后我们使用 $\mathtt{Cof}$ 在 $\forall(i : \ttI)$（公理 $\ax{8}$）下的封闭性在单独的步骤中修改这个合成。我们更喜欢这种方法，因为它简化了粘合的核心合成结构，并更明确地展示了 $\ax{8}$ 在立方类型论模型构造中的作用。
\end{remark}
...
这表明任何部分类型 $A$，如果在其定义的每个地方都同构于一个全类型 $B$，则可以扩展为一个全类型 $B'$，它同构于 $B$。我们在第 5.6.2 节中研究为什么立方体预层拓扑斯 \cite{18} 满足这个公理。给定 $\ax{9}$，定义一个严格形式的粘合是直接的。
...
本节的最后一步是使用公理 $\ax{8}$ 来调整 $\mathtt{SGlue}$ 的合成结构，以便我们恢复 $A$ 上的原始合成结构。
...
\begin{proof}
简单地取 $G = \mathtt{SGlue}\ \Phi\ A\ B\ f$；由定理 5.4.10 我们得到 $G$ 的合成结构，然后我们使用引理 5.3.10 来调整它以得到一个新的合成结构 $\gamma$ 满足所需的等式。
\end{proof}
