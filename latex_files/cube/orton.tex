...
\[
A \sim_{\mathcal{U}} B \triangleq \{P : \mathtt{Fib}(\Gamma \times \ttI) \mid P[\langle  id , 0\rangle] = A \land P[\langle  id , 1\rangle] = B\}
\]
\end{definition}
...
现在考虑以下情况：外部地，路径 $P : A \sim_{\mathcal{U}} B$ 对应于纤维化 $P \in \mathtt{FTy}_0(\Gamma.\ttI)$ 使得对于某个 $\Gamma \in \hatsquare$ 和 $A, B \in \mathtt{FTy}_0(\Gamma)$ 有 $P[\langle  id , 0\rangle] = A$ 和 $P[\langle  id , 1\rangle] = B$。从这些数据我们可以构造 $p \in \mathtt{Ter}(\Gamma \vdash \mathtt{Path}_{\mathcal{V}}\ \ulcorner A \urcorner\ \ulcorner B \urcorner)$ 如下：
\[
...
反之，给定 $p \in \mathtt{Ter}(\Gamma \vdash \mathtt{Path}_{\mathcal{V}}\ \ulcorner A \urcorner\ \ulcorner B \urcorner)$ 我们可以定义 $P \in \mathtt{FTy}_0(\Gamma.\ttI)$ 具有所需性质如下：
\[
...
&\quad(P : A \sim_{\mathcal{U}} B) \to (f : (x : \Gamma) \to \mathtt{fst}\ A\ x \to \mathtt{fst}\ B\ x) \times \mathtt{Equiv}\ f \notag
\end{align}
\end{theorem}
...
&\quad(f : (x : \Gamma) \to \mathtt{fst}\ A\ x \to \mathtt{fst}\ B\ x) \to (\mathtt{Equiv}\ f) \to A \sim_{\mathcal{U}} B \notag
\end{align}
\end{theorem}
