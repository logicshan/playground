...
\begin{frame}{一些记号}
  \begin{itemize}
    \item $A\in\Ty(A)$ 也记为 $\Gamma\vdash A$
    \item $a\in\Ter(\Gamma,A)$ 也记为 $\Gamma\vdash a:A$
    \item 态射 $\langle\gamma,a\rangle:\Delta\to\Gamma.A$ 可以刻画为
          \[
              \inferrule{\gamma:\Delta\to\Gamma \\ \Gamma\vdash A \\ \Delta\vdash a:A[\gamma]}{\langle\gamma,a\rangle:\Delta\to\Gamma.A}
          \]
    \item $[a_1,a_2,\ldots,a_n] : \Gamma \to \Gamma.A_1.A_2.\ldots A_n$ 归纳定义如下
      \begin{align*}
  [] &\triangleq id\\
  [a_1,\ldots,a_n,a_{n+1}] &\triangleq \langle[a_1,\ldots,a_n],a_{n+1}\rangle
  \end{align*}
  其中 $\Gamma.A_1.A_2.\ldots A_n\vdash A_{n+1}$, $\Gamma\vdash a_{n+1} : A_{n+1}[a_1,\ldots,a_n]$
  \end{itemize}
\end{frame}
...
\begin{frame}{续}
  \begin{itemize}
    ...
    =\Id_{A[\gamma\circ\ttp]}(a[\gamma\circ\ttp],\ttq)[\ttp]
