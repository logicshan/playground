...
\newcommand\realign{\mathtt{realign}}
...
\end{framed}
因此我们得到一个函数$\_[\_]:\Fib\Gamma\to(\Delta\to\Gamma)\to\Fib\Delta$, 定义为
\[
...
\begin{frame}[fragile,shrink]{纤维对象}
  我们称$A:\calU$是一个纤维对象, 如果我们有一个关于常数族$\lambda(\ul:1)\to A$ 在终对象 $1$ 上的纤维化结构. 注意: 如果$(A,\alpha):\Fib\Gamma$是一个纤维化, 那么对于每个$x:\Gamma$, 类型$A\,x:\calU$是纤维的, 其纤维化结构由通过映射$\gamma\triangleq(\lambda(\ul:1)\to x):1\to\Gamma$对$\alpha$进行重索引得到. $(A,\alpha)[\gamma]$就满足要求.

  \bigskip  

  然而, 反过来不成立: 拥有一个纤维化结构的族, 即$(x:\Gamma)\to\isFib(\lambda(\ul:1)\to A\,x)$ 的一个元素, 要弱于拥有一个对于 $A:\Gamma\to\calU$的纤维化结构.
\end{frame}

\begin{frame}[fragile,shrink]{分析}
\begin{itemize}
  \item $(A,\alpha):\Fib\Gamma$ 是一个纤维化
  \item $A:\Gamma\to\calU$, $\isFib\, A\triangleq(e:\{\ttzero,\ttone\})(p:\ttI\to\Gamma)\to\Comp\,e\,(A\circ p)$
  \item 对 $x : \Gamma$, 要证明 $\Gamma\, x$ 是纤维化的
  \item 我们证明 $\overline{A_x}\triangleq\lambda(\ul:\ttone)\to A\,x : \ttone\to\calU$ 上有一个纤维化结构
  \item $\gamma \triangleq \lambda(\ul:\ttone)\to x : \ttone \to \Gamma$
  \item 我们要证明 $(\overline{A_x},\alpha[\gamma]) \in \Fib\,\ttone$
  \item $(A,\alpha)[\gamma]$ 就满足要求
  \item 按定义, $\overline{A_x} = A\circ\gamma$
\end{itemize}
\end{frame}

\begin{frame}[fragile,shrink]{分析}
\begin{itemize}
  \item 我们要证明$(x:\Gamma)\to\isFib(\lambda(\ul:1)\to A\,x)$ 推不出 $A:\Gamma\to\calU$ 有一个纤维化结构
  \item 考虑 $P : \ttI \to \calU$, $P\ i \triangleq [\ttzero = i]$
  \item 下面我们证明对任意 $i : \ttI$, $P\ i : \calU$ 是一个纤维化对象
  \item $\rho_i : \isFib(\lambda(\ul:\ttone)\to P\ i)$, $\rho_i\ e\ p\ \phi\ f\ x\triangleq x$
  \item $\ttk Pi \triangleq \lambda(\ul:\ttone)\to P\ i$
  \item \begin{align*}\isFib\ \ttk Pi &\triangleq &&(e:\{\ttzero,\ttone\})(p:\ttI\to\ttone)\to\Comp\ e\ (\ttk Pi \circ p) \\
       &\triangleq&&(e:\{\ttzero,\ttone\})(p:\ttI\to\ttone)\to (\varphi:\Cof)(f:[\varphi]\to\Pi_{\ttI}(\ttk Pi \circ p)) \to\\
       &&&\{ a_0 : (\ttk Pi \circ p)\ e \mid (\varphi,f)\,@\,e \nearrow a_0 \} \to \\
       &&&\{a_1:(\ttk Pi \circ p)\,\overline{e}\mid(\varphi,f)\,@\,\overline{e}\nearrow a_1 \}\\
       &\triangleq&&(e:\{\ttzero,\ttone\})(p:\ttI\to\ttone)\to (\varphi:\Cof)(f:[\varphi]\to\Pi_{\ttI}(\ttk Pi \circ p)) \to\\
       &&&\{ a_0 : P\,i \mid (\varphi,f)\,@\,e \nearrow a_0 \} \to \\
       &&&\{a_1:P\,i \mid(\varphi,f)\,@\,\overline{e}\nearrow a_1 \}
  \end{align*}
\end{itemize}
\end{frame}

\begin{frame}[fragile,shrink]{分析(续)}
\begin{itemize}
  \item \begin{align*}\isFib\ P &\triangleq &&(e:\{\ttzero,\ttone\})(p:\ttI\to\ttI)\to\Comp\ e\ (P \circ p) \\
       &\triangleq&&(e:\{\ttzero,\ttone\})(p:\ttI\to\ttI)\to (\varphi:\Cof)(f:[\varphi]\to\Pi_{\ttI}(P \circ p)) \to\\
       &&&\{ a_0 : (P \circ p)\ e \mid (\varphi,f)\,@\,e \nearrow a_0 \} \to \\
       &&&\{a_1:(P \circ p)\,\overline{e}\mid(\varphi,f)\,@\,\overline{e}\nearrow a_1 \}
  \end{align*}
  \item 如果存在 $\rho : \isFib\ P$
  \item 对任意$B:\calU$, $\elim_{\emptyset} : [\bot]\to B$
  \item \begin{align*}\rho\ \ttzero\ id\ \elim_{\emptyset} : &\{a_0:(P\circ id) \ttzero \mid (\bot,\elim_{\emptyset})@\ttzero\nearrow a_0\} \to \\
  &\{ a_1:(P\circ id) \ttone \mid (\bot,\elim_{\emptyset})@\ttone\nearrow a_1 \} \end{align*}
\end{itemize}
\end{frame}

\begin{frame}[fragile,shrink]{分析(续)}
\begin{itemize}
  \item $(\bot,\elim_{\emptyset})@\ttzero = (\bot,\lambda(u:[\bot])\to \elim_{\emptyset}\ u\ i)$
  \item \begin{align*}(\bot,\elim_{\emptyset})@\ttzero\nearrow a_0 &= (\bot,\lambda(u:[\bot])\to \elim_{\emptyset}\ u\ i)\nearrow a_0 \\
  &= \forall(u:[\bot]).(\lambda(u:[\bot])\to \elim_{\emptyset}\ u\ i)u = a_0 \\
  &=\forall(u:[\bot]).\elim_{\emptyset}\ u\ i = a_0\\
  &=\elim_{\emptyset}
\end{align*}
  \item \begin{align*}\rho\ \ttzero\ id\ \elim_{\emptyset} : &\{a_0:[\ttzero=\ttzero] \mid (\bot,\elim_{\emptyset})@\ttzero\nearrow a_0\} \to \\
  &\{ a_1:[\ttzero=\ttone] \mid (\bot,\elim_{\emptyset})@\ttone\nearrow a_1 \} \end{align*}
  \item \[\rho\ \ttzero\ id\ \elim_{\emptyset} : \{\ast\} \to \{ a_1:[\ttzero=\ttone] \mid (\bot,\elim_{\emptyset})@\ttone\nearrow a_1 \} \]
  \item 那么 $\rho\ \ttzero\ id\ \bot\ \elim_{\emptyset}\ \ast : [\ttzero = \ttone]$
  \item 这和 $\ax{2}$ 矛盾
\end{itemize}
\end{frame}
...
\begin{frame}[fragile,shrink]{分析}
\begin{itemize}
  \item $e : \{ \ttzero, \ttone \}$, $A:\ttI\to\calU$
  \item \begin{align*}
\mathtt{Fill}\,e\,A \triangleq &(\varphi : \mathtt{Cof})(f : [\varphi] \to \Pi_\mathtt{I} A)(a : \{a' : A\,e \mid (\varphi, f) \ @\  e \nearrow a'\}) \to \\
...
\end{align*}
  \item $\alpha : \Fill\ e\ A$
  \item $(\lambda\varphi\ f\ a \to \alpha\ \varphi\ f\ a\ \overline{e}) : \Comp\ e\ A$
\end{itemize}
\end{frame}
...
\begin{frame}[fragile,shrink]{纤维化的性质}
  ...
  \begin{itemize}
    \item $A:\Gamma\to\calU$
     \begin{align*}
      \alpha :\isFib\ A \triangleq &(e:\{\ttzero,\ttone\})(p:\ttI\to\Gamma)\to(\varphi:\Cof)(f:[\varphi]\to\Pi_{\ttI}(A\circ p))\to\\
      &\{a_0:(A\circ p)\ e\mid(\varphi,f)@e\nearrow a_0\}\to\{a_1:(A\circ p)\ \overline{e}\mid(\varphi,f)@\overline{e}\nearrow a_1\}\end{align*}
       \begin{align*}
      \beta :\isFib\ B \triangleq &(e:\{\ttzero,\ttone\})(p:\ttI\to\Gamma)\to(\varphi:\Cof)(f:[\varphi]\to\Pi_{\ttI}(B\circ p))\to\\
      &\{a_0:(B\circ p)\ e\mid(\varphi,f)@e\nearrow a_0\}\to\{a_1:(B\circ p)\ \overline{e}\mid(\varphi,f)@\overline{e}\nearrow a_1\}\end{align*}
      $A \cong B \triangleq (x : \Gamma) \to A\,x \cong B\,x$
    \item $\alpha\ e\ p\ \varphi\ q\ a_0\triangleq f^{-1}\,(p\ \overline{e})\,(\beta\ e\ p\ \varphi\ (\lambda u\ i\to f\ (p\ i)\ (q\ u\ i))\ (f\ (p\ e)\ a_0))$
  \end{itemize}
\end{frame}


\begin{frame}[fragile,shrink]{上纤维部分族(Cofibrant-partial families)}
  \begin{itemize}
    \item $\Gamma : \calU$, $\Phi : \Gamma \to \Cof$
    \item $\Gamma|\Phi \triangleq (x:\Gamma)\times[\Phi\ x]$
    \item $\iota : \Gamma|\Phi \rightarrowtail \Gamma$ (就是投影映射)
    \item $\Gamma$ 限制在 $\Phi$ 上的一个上纤维部分族定义为 $A: (\Gamma|\Phi)\to\calU$
  \end{itemize}

  对齐引理: 对$A : \Gamma \to \calU$, $\beta : \isFib(A\circ\iota)$, $\alpha : \isFib\ A$, 存在一个复合结构 $\realign(\Phi,\beta,\alpha) : \isFib\ A$, 使得 $\beta = \realign(\Phi,\beta,\alpha)[\iota]$
\end{frame}
...
\begin{frame}[fragile,shrink]
     \begin{align*}
      ...
      &\{a_0:(A\circ p)\ e\mid(\varphi,f)@e\nearrow a_0\}\to\{a_1:(A\circ p)\ \overline{e}\mid(\varphi,f)@\overline{e}\nearrow a_1\}\end{align*}
       \begin{align*}
      \beta :\isFib\ (A\circ\iota) \triangleq &(e:\{\ttzero,\ttone\})(p:\ttI\to\Gamma|\Phi)\to(\varphi:\Cof)(f:[\varphi]\to\Pi_{\ttI}((A\circ\iota)\circ p))\to\\
      &\{a_0:((A\circ\iota)\circ p)\ e\mid(\varphi,f)@e\nearrow a_0\}\to\\
      &\{a_1:((A\circ\iota)\circ p)\ \overline{e}\mid(\varphi,f)@\overline{e}\nearrow a_1\}
    \end{align*}
    $ \realign(\Phi, \beta, \alpha)\ e\ p\ \psi\ f\ g \triangleq \alpha\ e\ p\ (\psi \lor (\forall(i : \mathtt{I}).\ \Phi\ (p\ i)))\ (f \cup f')\ g $

    其中

    \begin{align*}
      f' : [\forall(i : \mathtt{I}).\ \Phi\ (p\ i)] &\to \Pi_{\mathtt{I}}(A \circ p)\\
    u &\mapsto \mathtt{fill}\ e\ \beta\ (\lambda i \to (p, u\ i))\ \psi\ f\ g 
  \end{align*}
\end{frame}
\end{document}