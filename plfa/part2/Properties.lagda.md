...
open import plfa.part1.Isomorphism hiding (_∘_)
...
Canonical-≃ =
  ...
  to (C-suc canonical) with to canonical
  ...                     |  ⟨ t , v ⟩   = ⟨ ⊢suc t , V-suc v ⟩
  from : ∀ {V A} → (∅ ⊢ V ⦂ A) × (Value V) → Canonical V ⦂ A
  from ⟨ ⊢ƛ t , V-ƛ ⟩ = C-ƛ t
  from ⟨ ⊢zero , V-zero ⟩ = C-zero
  from ⟨ ⊢suc t , V-suc v ⟩ = C-suc (from ⟨ t , v ⟩)
  from∘to : ∀ {V A} → (canonical : Canonical V ⦂ A) → from (to canonical) ≡ canonical
  from∘to (C-ƛ x) = refl
  from∘to C-zero = refl
  from∘to (C-suc canonical) = cong C-suc (from∘to canonical)
  to∘from : ∀ {V A} → (t×v : (∅ ⊢ V ⦂ A) × (Value V)) → to (from t×v) ≡ t×v
  to∘from ⟨ ⊢ƛ t , V-ƛ ⟩ = refl
  to∘from ⟨ ⊢zero , V-zero ⟩ = refl
  to∘from ⟨ ⊢suc t , V-suc v ⟩ = cong (λ{ ⟨ t , v ⟩ → ⟨ ⊢suc t , V-suc v ⟩}) (to∘from ⟨ t , v ⟩)
```
...
Progress-≃ =
  ...
  from (inj₁ x) = done x
  from (inj₂ ⟨ v , s ⟩) = step s
  from∘to : ∀ {M} → (progress : Progress M) → from (to progress) ≡ progress
  from∘to (step x) = refl
  from∘to (done x) = refl
...
```agda
progress′′ : ∀ M {A} → ∅ ⊢ M ⦂ A → Value M ⊎ ∃[ N ](M —→ N)
progress′′ M = (_≃_.to Progress-≃) ∘ progress

progress′ : ∀ M {A} → ∅ ⊢ M ⦂ A → Value M ⊎ ∃[ N ](M —→ N)
progress′ (ƛ x ⇒ M) (⊢ƛ T) = inj₁ V-ƛ
progress′ (M · M₁) (T · T₁) with progress′ M T
... | inj₂ ⟨ M′ , M—→M′ ⟩ = inj₂ ⟨ M′ · M₁ , ξ-·₁ M—→M′ ⟩
... | inj₁ v₁ with progress′ M₁ T₁
progress′ ((ƛ x ⇒ M) · M₁) (T · T₁) | inj₁ V-ƛ | inj₁ v₂ = inj₂ ⟨ M [ x := M₁ ] , β-ƛ v₂ ⟩
...             | inj₂ ⟨ fst , snd ⟩ = inj₂ ⟨ (M · fst) , (ξ-·₂ v₁ snd) ⟩
progress′ `zero ⊢zero = inj₁ V-zero
progress′ (`suc M) (⊢suc T) with progress′ M T
... | inj₁ x = inj₁ (V-suc x)
... | inj₂ ⟨ fst , snd ⟩ = inj₂ ⟨ (`suc fst) , (ξ-suc snd) ⟩
progress′ case M [zero⇒ M₁ |suc x ⇒ M₂ ] (⊢case T T₁ T₂) with progress′ M T
... | inj₂ ⟨ fst , snd ⟩ = inj₂ ⟨ case fst [zero⇒ M₁ |suc x ⇒ M₂ ] , (ξ-case snd) ⟩
... | inj₁ V-zero = inj₂ ⟨ M₁ , β-zero ⟩
... | inj₁ (V-suc {V} x₁) = inj₂ ⟨ M₂ [ x := V ] , β-suc x₁ ⟩
progress′ (μ x ⇒ M) (⊢μ T) = inj₂ ⟨ (M [ x := μ x ⇒ M ]) , β-μ ⟩
```
...
unstuck : ∀ {M A}
  → ∅ ⊢ M ⦂ A
    -----------
  → ¬ (Stuck M)
unstuck w with progress w
... | step M—→N = λ{ ⟨ normal , ¬value ⟩ → normal M—→N }
... | done value = λ{ ⟨ normal , ¬value ⟩ → ¬value value }
...
preserves : ∀ {M N A}
  ...
  → ∅ ⊢ N ⦂ A
preserves w (_ ∎) = w
preserves w (L —→⟨ L—→M ⟩ M—↠N) = preserves (preserve w L—→M) M—↠N
...
wttdgs : ∀ {M N A}
  ...
  → M —↠ N
    -----------
  → ¬ (Stuck N)
wttdgs w M—↠N = unstuck (preserves w M—↠N)
```
