...
inv-s≤s' (s≤s suc-m≤suc-n) = suc-m≤suc-n
```
...
suc-total : ∀ (m n : ℕ) → Total2 m n → Total2 (suc m) (suc n)
suc-total m n (lt x) = lt (s<s x)
suc-total m n (eq x) = eq (cong suc x)
suc-total m n (gt x) = gt (s>s x)

trichotomy : ∀ (m n : ℕ) → Total2 m n
trichotomy zero zero = eq refl
trichotomy zero (suc n) = lt z<s
trichotomy (suc m) zero = gt s>z
trichotomy (suc m) (suc n) = suc-total m n (trichotomy m n)
```
...
≤→< (suc m) (suc n) suc-m≤n = s<s (≤→< m n (inv-s≤s suc-m≤n))
...
<→≤ (suc m) (suc n) m<n = s≤s (<→≤ m n (inv-s<s m<n))
```
...
<suc-n→≤n : ∀ (m n : ℕ) → m < suc n → m ≤ n
<suc-n→≤n zero n m<suc-n = z≤n
<suc-n→≤n (suc m) n m<suc-n = <→≤ m n (inv-s<s m<suc-n)

<-trans-revisited : ∀ {m n p : ℕ} → m < n → n < p → m < p
<-trans-revisited {zero} {suc n} {suc p} m<n n<p = z<s
<-trans-revisited {suc m} {suc n} {suc p} m<n n<p = s<s m<p
  where
  suc-m≤n : suc m ≤ n
  suc-m≤n = <suc-n→≤n (suc m) n m<n
  n≤p : n ≤ p
  n≤p = <⇀≤ (inv-s<s n<p)
  suc-m≤p : suc m ≤ p
  suc-m≤p = ≤-trans suc-m≤n n≤p
  m<p : m < p
  m<p = ≤→< m p suc-m≤p
```
...
data One : Bin → Set where
  ...
  tailO : ∀ {b : Bin} → One b → One (b O)

  tailI : ∀ {b : Bin} → One b → One (b I)
...
popO : ∀ {b : Bin} → One (b O) → One b
popO (tailO One-bO) = One-bO

one-inc : ∀ {b : Bin} → Can b → One (inc b)
one-inc zero = one
one-inc {b O} (can x) = tailI (popO x)
one-inc {.⟨⟩ I} (can one) = tailO one
one-inc {b I} (can (tailI x)) = tailO (one-inc (can x))

can-inc : ∀ {b : Bin} → Can b → Can (inc b)
can-inc can-b = can (one-inc can-b)
...
1≤from-one (tailO {b} one-b) rewrite +-comm (from b) (from b + zero) =
  +-mono-≤ 0 (from b + 0) 1 (from b) z≤n (1≤from-one one-b)
1≤from-one (tailI one-b) = s≤s z≤n

2*suc→tailO : ∀ (n : ℕ) → to (2 * (suc n)) ≡ (to (suc n)) O
2*suc→tailO zero = refl
2*suc→tailO (suc n) rewrite +-suc n (suc (n + zero))
                          | 2*suc→tailO n = refl

2*→tailO : ∀ {n : ℕ} → 1 ≤ n → to (2 * n) ≡ (to n) O
2*→tailO {suc n} _ = 2*suc→tailO n

2*suc→tailI : ∀ (n : ℕ) → to (suc (2 * (suc n))) ≡ (to (suc n)) I
2*suc→tailI zero = refl
2*suc→tailI (suc n) rewrite +-suc n (suc (n + zero))
                          | 2*suc→tailI n = refl

2*→tailI : ∀ {n : ℕ} → 1 ≤ n → to (suc (2 * n)) ≡ (to n) I
2*→tailI {suc n} 1≤n = 2*suc→tailI n
...
to∘from≡id (can one) = refl
to∘from≡id (can (tailO {b} x)) rewrite sym (to∘from≡id {b} (can x))
                                     | from∘to≡id (from b)
           = 2*→tailO (1≤from-one x)
to∘from≡id (can (tailI {b} x)) rewrite sym (to∘from≡id {b} (can x))
                                     | from∘to≡id (from b)
           = 2*→tailI (1≤from-one x)
