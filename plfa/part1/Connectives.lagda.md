...
⊎-comm' = 
  record 
    { to      = helper-to
    ; from    = helper-from
    ; from∘to = helper-from∘to
    ; to∘from = helper-to∘from
    }
  where
  helper-to : ∀ {A B : Set} → A ⊎ B → B ⊎ A
  helper-to =  λ{ A⊎B → case-⊎ inj₂ inj₁ A⊎B }
  helper-from : ∀ {A B : Set} → B ⊎ A → A ⊎ B
  helper-from = λ{ B⊎A → case-⊎ inj₂ inj₁ B⊎A }
  helper-from∘to : ∀ {A B : Set} → (x : A ⊎ B) → helper-from (helper-to x) ≡ x
  helper-from∘to (inj₁ _) = refl
  helper-from∘to (inj₂ _) = refl
  helper-to∘from : ∀ {A B : Set} → (x : B ⊎ A) → helper-to (helper-from x) ≡ x
  helper-to∘from (inj₁ _) = refl
  helper-to∘from (inj₂ _) = refl
```
...
{-
x⊎-implies-⊎x : ∀ {A B C D : Set} → (A ⊎ C) × (B ⊎ D) → (A × B) ⊎ (C × D)
x⊎-implies-⊎x ⟨ inj₁ a , inj₁ b ⟩ = inj₁ ⟨ a , b ⟩
x⊎-implies-⊎x ⟨ inj₁ a , inj₂ d ⟩ = {!!}
x⊎-implies-⊎x ⟨ inj₂ c , inj₁ b ⟩ = {!!}
x⊎-implies-⊎x ⟨ inj₂ c , inj₂ d ⟩ = inj₂ ⟨ c , d ⟩
-}
```
