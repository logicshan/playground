...
open import plfa.part1.Relations using (_<_; z<s; s<s; inv-s<s; inv-s≡s)
...
suc m <? suc n with m <? n
...               | yes m<n = yes (s<s m<n)
...               | no ¬m<n = no λ{ sm<sn → ¬m<n (inv-s<s sm<sn) }
```
...
Show that both of the above are decidable.
    ...
    Can? : ∀ (b : Bin) → Dec (Can b)
```agda
open import plfa.part1.Relations using (Bin; ⟨⟩; _O; _I; to; from; One; one; tailO; tailI; Can; zero; can)

One? : ∀ (b : Bin) → Dec (One b)
One? ⟨⟩ = no (λ ())
One? (b O) with One? b
... | yes one-b = yes (tailO one-b)
... | no ¬one-b = no λ{ (tailO one-b) → ¬one-b one-b }
One? (b I) with One? b
... | yes one-b = yes (tailI one-b)
One? (⟨⟩ I)    | no ¬one-b  = yes one
One? ((b O) I) | no ¬one-bO = no λ{ (tailI one-bO) → ¬one-bO one-bO }
One? ((b I) I) | no ¬one-bI = no λ{ (tailI one-bI) → ¬one-bI one-bI }

Can? : ∀ (b : Bin) → Dec (Can b)
Can? ⟨⟩ = no λ{ (can ()) }
Can? (b O) with Can? b
... | yes zero = no λ{ (can (tailO (tailO ()))) }
... | yes (can one-b) = yes (can (tailO one-b))
Can? (⟨⟩ O) | no x = yes zero
Can? ((b O) O) | no ¬can-bO = no λ{ (can (tailO one-bO)) → ¬can-bO (can one-bO) }
Can? ((b I) O) | no ¬can-bI = no λ{ (can (tailO one-bI)) → ¬can-bI (can one-bI) }
Can? (b I) with Can? b
... | yes zero = no λ{ (can (tailI (tailO ()))) }
... | yes (can one-b) = yes (can (tailI one-b))
Can? (⟨⟩ I) | no x = yes (can one)
Can? ((b O) I) | no ¬can-bO = no λ{ (can (tailI one-bO)) → ¬can-bO (can one-bO) }
Can? ((b I) I) | no ¬can-bI = no λ{ (can (tailI one-bI)) → ¬can-bI (can one-bI) }
```
