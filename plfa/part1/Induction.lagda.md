...
0^suc-n≡0 : ∀ (n : ℕ) → 0 ^ (suc n) ≡ 0
0^suc-n≡0 n = refl

1^n≡1 : ∀ (n : ℕ) → 1 ^ n ≡ 1
1^n≡1 zero = refl
1^n≡1 (suc n) rewrite +-identityʳ (1 ^ n) = 1^n≡1 n
...
^-distribʳ-* m n (suc p) =
  ...
  ≡⟨ *-assoc m n ((m * n) ^ p) ⟩
    m * (n * ((m * n) ^ p))
  ≡⟨ cong (m *_) (*-comm n ((m * n) ^ p)) ⟩
    m * (((m * n) ^ p) * n)
  ≡⟨ cong (m *_) (cong (_* n) (^-distribʳ-* m n p)) ⟩
    m * (((m ^ p) * (n ^ p)) * n)
  ≡⟨ cong (m *_) (*-assoc (m ^ p) (n ^ p) n) ⟩
    m * ((m ^ p) * ((n ^ p) * n))
  ≡⟨ cong (m *_) (cong ((m ^ p) *_) (*-comm (n ^ p) n)) ⟩
    m * ((m ^ p) * (n * (n ^ p)))
...
^-*-assoc m (suc n) p rewrite ^-distribˡ-+-* m p (n * p)
                            | ^-distribʳ-* m (m ^ n) p
                            | ^-*-assoc m n p = refl
```
...
to∘from≡id ⟨⟩ = {!!}
to∘from≡id (b O) rewrite +-identityʳ (from b) = {!!}
to∘from≡id (b I) = {!!}
-}
